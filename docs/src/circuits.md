...
包内置两个一维最近邻变分 ansatz（hardware efficient），作为「先建线路、
后绑参」工作流（§3）的参考示例，源码见 `src/variational.jl`：

| 函数 | 旋转层（每比特） | 参数个数 |
|---|---|---|
| `variational_circuit_1d(L, depth)` | `RZ-RY-RZ` | `3L(depth+1)` |
| `real_variational_circuit_1d(L, depth)` | `RY` | `L(depth+1)` |

相邻旋转层之间铺一维最近邻 `CX` 梳（共 `depth` 个纠缠层）：奇数层正序
`1→L-1`、偶数层倒序交替。`θs` 缺省时线路携带符号参数 `θ[1]…θ[n]`
（`params(:θ, n)` 向量）；也可直接传 `Vector{<:Real}` 一步绑定数值。

```julia
c = variational_circuit_1d(2, 1)                     # 符号参数 θ[1]…θ[12]
bound = assign(c, Dict(params(:θ, 12) => collect(0.1:0.1:1.2)))
r = real_variational_circuit_1d(3, 1; θs = rand(6) .* 2π)
```

`variational_circuit_1d(2, 1)` 的线路图（`draw(c)`）：

```
q[1]: ─RZ(:θ[1])──RY(:θ[2])──RZ(:θ[3])───────────────────────────────────●──RZ(:θ[7])──RY(:θ[8])──RZ(:θ[9])─────────────────────────────────────
                                                                         │
q[2]: ──────────────────────────────────RZ(:θ[4])──RY(:θ[5])──RZ(:θ[6])──X───────────────────────────────────RZ(:θ[10])──RY(:θ[11])──RZ(:θ[12])─
```

`real_variational_circuit_1d(3, 1)` 的线路图——注意单个纠缠层内 `CX` 梳
正序铺满（`1→2→3`），`depth ≥ 2` 时下一层倒序交替：
