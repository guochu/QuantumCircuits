@testset "qasm2 roundtrip" begin
    ...
    @test num_ops(c2) == 4
    @test c2.ops[1] == GateOp(H, [1], [])
    @test c2.ops[2] == GateOp(CX, [1, 2], [])
    # 参数门
    c3 = Circuit(2)
    push!(c3, RX(0.5, 1)); push!(c3, RZZ(1.2, 1, 2)); push!(c3, PHASE(0.3, 1))
    s3 = to_qasm(c3; version=2)
    @test occursin("rx(0.5) q[0];", s3)
    @test occursin("rzz(1.2) q[0],q[1];", s3)
    c4 = read_qasm(s3)
    @test c4.ops[1] == GateOp(RX, [1], [0.5])
    @test c4.ops[2] == GateOp(RZZ, [1, 2], [1.2])
    # u1/u3/cu1 映射
    src = """
    OPENQASM 2.0;
    include "qelib1.inc";
    ...
    """
    c6 = read_qasm(src2)
    @test c6.ops == Operation[GateOp(H, [1], []), GateOp(CX, [1, 2], [])]
    # if 语句
    src3 = """
    OPENQASM 2.0;
    include "qelib1.inc";
...
@testset "qasm3 roundtrip" begin
    ...
    @test c2.ops[3] == MeasOp([1], [c.cregs[1][1]])
    @test c2.ops[4] == MeasOp([2], [c.cregs[1][2]])
    # 修饰符导出/回读
    c3 = Circuit(3)
    push!(c3, ctrl(H(1), 2))
    push!(c3, negctrl(X(1), 1, 3))
    push!(c3, inv(RX(0.4, 1)))
    push!(c3, pow(S(1), 2))
    s3 = to_qasm(c3; version=3)
    @test occursin("ctrl @ h q[1], q[0];", s3)
    c4 = read_qasm(s3)
    ...
    @test c10.ops[1] == BarrierOp([1, 2])
    @test c10.ops[2] == ReinitOp([1])
end
